...
export function ProductAdminTable({
	...
	const columns = [
		...
		columnHelper.accessor("priceLabel", {
			header: ({ column }) => (
				<DataGridColumnHeader title="Preço" column={column} />
			),
			enableSorting: true,
			size: 160,
			...
			cell: ({ row }) => {
				...
				return (
					<Button
						...
						disabled={isSaving}
						onClick={() =>
							void onChangeStatus(product, nextStatus)
						}
					>
						{product.status === "published" ? (
	...
	if (products.length === 0) {
		return (
			<div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">
				Nenhum produto criado. Adicione o primeiro item para transformar a
				vitrine em algo conversável.
			</div>
		);
	}
