...
import { type ProductStatusFilter, ProductToolbar } from "#/features/product/ui/ProductToolbar"
...
export function ProductDataGrid({ products, listSearch, onCreate, onEdit }: ProductDataGridProps) {
  ...
  useEffect(() => {
    setColumnFilters(listSearch.status ? [{ id: "status", value: listSearch.status }] : [])
  }, [listSearch.status])
    ...
    onPaginationChange: (updater) => {
      ...
      void navigate({
        search: (previous) => ({
          ...
          pageSize: next.pageSize === PAGE_SIZE ? undefined : (next.pageSize as 25 | 50),
        }),
      })
    },
  ...
  return (
    <div className="grid gap-3">
      ...
      <DataGrid
        emptyMessage={
          hasActiveFilters ? "Nenhum produto corresponde aos filtros." : "Nenhum produto criado."
        }
        onRowClick={onEdit}
