...
export const saveCatalogIdentityToDatabase = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<MutationResult> => {
		...
		try {
			...
			if (description && description.length > 160) return { ok: false, message: "A descrição deve ter no máximo 160 caracteres." };
			const coverImageUrl = data.coverImageUrl?.trim() || undefined;
			if (coverImageUrl && !/^https?:\/\//.test(coverImageUrl)) return { ok: false, message: "Informe uma URL pública válida para a imagem de capa." };
			if (data.catalogId) {
				await db
					.update(catalogs)
					.set({
			...
			return {
				ok: false,
				message:
					"Nao foi possivel salvar. Verifique o PostgreSQL e se o slug ja esta em uso.",
			};
		}
	});
	...
	.handler(async ({ data }): Promise<MutationResult> => {
		try {
			...
			const visibleProducts = await db
				...
				.where(
					and(
						eq(products.catalogId, data.catalogId),
