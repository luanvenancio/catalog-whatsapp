...
export function parseCatalogName(name: string): Result<string, CatalogInputError> {
...
export function parseCatalogSlug(slug: string): Result<CatalogSlug, CatalogInputError> {
  ...
  if (!publicSlugPattern.test(normalizedSlug)) {
    return {
      ...
      error: {
        ...
        message: "O identificador público deve usar letras minúsculas, números e hífens.",
      },
    }
  }
...
export function parseWhatsappNumber(whatsappNumber: string): Result<string, CatalogInputError> {
