...
export type StoredCatalogDraftState = { status: "empty" } | { status: "ready"; catalog: Catalog }
...
export function saveCatalogDraft(catalog: Catalog): StoredCatalogDraftState {
  if (typeof window !== "undefined") {
    window.localStorage.setItem(catalogDraftStorageKey, JSON.stringify(catalog))
  }
