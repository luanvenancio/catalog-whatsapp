...
import { catalogIdentityFormSchema, type CatalogIdentityFormValues } from "#/features/catalog/ui/forms/CatalogIdentityFormSchema";
...
export function CreateCatalogForm({ isSaving, onSubmit }: CreateCatalogFormProps) {
	...
	return (
		...
		>
			...
			<form.Field name="whatsappNumber" validators={{ onBlur: catalogIdentityFormSchema.shape.whatsappNumber }}>
				{(field) => {
					const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid;
					return <Field data-invalid={isInvalid}>
				...
				{(field) => {
					...
					return <Field data-invalid={isInvalid}>
						...
						<Input
							...
							value={field.state.value}
						/>
						<FieldDescription>Seu link publico sera /catalogs/{field.state.value || "bela-fatia"}.</FieldDescription>
						<FieldError errors={field.state.meta.errors} id={`${field.name}-error`} />
					</Field>
				}}
			</form.Field>
