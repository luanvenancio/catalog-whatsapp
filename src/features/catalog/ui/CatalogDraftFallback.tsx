...
type DraftPreviewState = { status: "empty" } | { status: "ready"; catalogName: string }
    ...
    return (
      <main className="flex min-h-dvh items-center justify-center bg-zinc-50 px-4">
        <section className="max-w-md rounded-lg border border-zinc-200 bg-white p-6 text-center shadow-sm">
          <p className="text-sm font-semibold text-emerald-700">Rascunho encontrado</p>
          <h1 className="mt-2 text-2xl font-bold text-zinc-950">{draftPreviewState.catalogName}</h1>
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-zinc-50 px-4">
      <section className="max-w-md rounded-lg border border-zinc-200 bg-white p-6 text-center shadow-sm">
        ...
        <h1 className="mt-2 text-2xl font-bold text-zinc-950">Vitrine não encontrada</h1>
        <p className="mt-2 text-zinc-600">
...
function readUrlDraftPreview(): DraftPreviewState {
  ...
  const draftName = new URLSearchParams(window.location.search).get("draftName")
  if (!draftName) {
    return { status: "empty" }
  }
