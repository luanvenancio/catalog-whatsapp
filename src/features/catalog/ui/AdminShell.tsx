...
export function AdminShell({ businessName = "Minha vitrine", children }: AdminShellProps) {
      ...
      <header className="border-b bg-background">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-3">
          <Link activeOptions={{ exact: true }} className="min-w-0" to="/admin">
            <span className="block truncate text-lg font-semibold">{businessName}</span>
            <span className="block text-xs text-muted-foreground">Administracao</span>
          </Link>
        </div>
        <nav
